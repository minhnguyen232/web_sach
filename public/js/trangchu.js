				...
				switch (tenThanhPhanCanSort) {
					case 'price':
						...
						result.sort(function (a, b) {
							var giaA = parseInt(a.price);
							var giaB = parseInt(b.price);
							return giaA - giaB;
						});
						break;
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
	...
	s += `	</div>
			<a class="xemTatCa" href="/product?` + filter.join('&') + `" style="` + borderA + `">
...
function timKiemTheoKhuyenMai(list, tenKhuyenMai, soluong) {
	...
	for (var i = 0; i < list.length; i++) {
		const listSet = JSON.parse(list[i].promo);
		if (listSet.name == tenKhuyenMai) {
			result.push(list[i]);
			count--;
			if (count <= 0) break;
		}
...
function createLinkFilter(type, nameFilter, valueAdd) {
	...
	var link = 'product'; //window.location.href.split('?')[0].replace('#', '');
