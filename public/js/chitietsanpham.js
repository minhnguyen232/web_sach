...
function phanTich_URL_chiTietSanPham() {
    ...
    const product = list_products.find((product) => product.id == id);
    nameProduct = product.name; // lấy tên
    ...
    const detail = JSON.parse(sanPhamHienTai.detail);
    var s = addThongSo('Tác giả', detail.size);
    s += addThongSo('Xuất xứ', detail.xuatxu);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    const promo = JSON.parse(sp.promo)
    switch (promo.name) {
        case 'giamgia':
            var span = `<span style="font-weight: bold">` + promo.value + `</span>`;
            return `Khách hàng sẽ được giảm ` + span + `₫ khi tới mua trực tiếp tại cửa hàng`;

        case 'moiramat':
            return `Khách hàng được đọc tại cửa hàng.`;

        case 'giareonline':
            var del = sp.price - stringToNum(promo.value);
...
function suggestion() {
    ...
    const sanPhamTuongTu = list_products
        // Lọc sản phẩm trùng
        .filter((_) => _.code !== sanPhamHienTai.code)
