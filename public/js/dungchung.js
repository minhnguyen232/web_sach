...
function groupOrder(data) {
    ...
    for (const order of data) {
        const { order_id, product_code, quantity, status, createdAt, user } = order;
...
function themVaoGioHang(code, tensp) {
    var user = getUser();
    if (!user) {
    ...
    setUser(user); // cập nhật giỏ hàng cho user hiện tại
    capNhat_ThongTin_User(); // cập nhật giỏ hàng
}
...
function getUser() {
    return JSON.parse(window.localStorage.getItem('user')); // Lấy dữ liệu từ localstorage
}
...
// Check xem có ai đăng nhập hay chưa (User có hay chưa)
...
function capNhat_ThongTin_User() {
    var u = getUser();
    if (u) {
...
function addProduct(p, ele, returnString) {
    const promoSet = JSON.parse(p.promo);
    promo = new Promo(promoSet.name, promoSet.value); // class Promo
    product = new Product(p.id, p.code, p.name, p.img, p.price, p.star, p.rateCount, promo); // Class product
            ...
            <div class="tools-member">
                <div class="member">
                    ...
                    <div class="menuMember hide">
                        <a href="/profile">Trang người dùng</a>
            ...
            <div class="tab-content">
                <div id="login">
                    ...
                    <form>
                        ...
                        <button type="button" class="button button-block" onclick="logIn()" />Tiếp tục</button>
                ...
                <div id="signup">
                    ...
                    <form >
                        ...
                        <div class="top-row">
                            <div class="field-wrap">
                                <label>
                                    Tên người dùng<span class="req">*</span>
                                </label>
                                <input name="username" type="text" required autocomplete="off" />
                            </div>
                        ...
                        <div class="field-wrap">
                            <label>
                                Tên đẩy đủ<span class="req">*</span>
                            </label>
                        ...
                        <button type="button" class="button button-block" onclick="signUp()" />Tạo tài khoản</button>
