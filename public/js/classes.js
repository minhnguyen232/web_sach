...
function Product(id, code, name, img, price, star, rateCount, promo) {
	this.id = id;
	this.code = code;
	this.img = img;
	this.name = name;
...
function addToWeb(p, ele, returnString) {
	...
	if (promo && promo.name == "giareonline") {
	...
	var chitietSp = '/product/detail/' + p.id;
	...
	var newLi =
		...
		<a href="` + chitietSp + `">
			...
			<div class="tooltip">
				<button class="themvaogio" onclick="themVaoGioHang('`+ p.code + `', '` + p.name + `'); return false;">
